...
module Church.InitialAlgebrasCont2 where
...
--The naturality lemma is continuous in the function.
...
module InitialOfFunctor (idDummy :{id} Set) (#Dummy :{#} Set) where
  ...
  module NaturalityContProven {ℓ :{¶} Level} where
    postulate
      A B :{#} Set ℓ
      mkA : IsAlg A
      mkB : IsAlg B
    ...
    open Naturality mk[ mkA fibred-over mkB by mf ] mkB get-image (refl _)
      renaming (naturality to naturality-get-image)

    open Naturality mk[ mkA fibred-over mkB by mf ] mkA unfibre (refl _)
      renaming (naturality to naturality-unfibre)

    naturality :{¶} f ∘ fold mkA ≡ fold mkB
    naturality =
      (f ∘ fold mkA ≡ f ∘ unfibre ∘ fold mk[ mkA fibred-over mkB by mf ]) ∋
        cong (λ g → f ∘ g) (sym naturality-unfibre) •
      (f ∘ unfibre ∘ fold mk[ mkA fibred-over mkB by mf ] ≡ get-image ∘ fold mk[ mkA fibred-over mkB by mf ]) ∋
        cong (λ g → g ∘ fold mk[ mkA fibred-over mkB by mf ]) (fib-lemma f) •
      (get-image ∘ fold mk[ mkA fibred-over mkB by mf ] ≡ fold mkB) ∋
        naturality-get-image

  module NaturalityCont {ℓ :{¶} Level}
      {A B :{#} Set ℓ}
      (mkA : IsAlg A)
      (mkB : IsAlg B)
      (f : A → B)
  ...
  module LoweringLemma where
    module Core {ℓ :{¶} Level} {X :{#} Set ℓ} (mkX : IsAlg X) where
      ...
      open NaturalityCont {lsuc ℓ} mkMu (mkLift mkX) lift∘fold (refl _)
  ...
  module InitialityProven {ℓ :{¶} Level} where
    ...
    open NaturalityCont {lsuc ℓ} mkMu mkB f (refl _)
...
module InitialOfIndexedFunctor {ℓZ :{¶} Level} (Z :{#} Set ℓZ) (idDummy :{id} Set) (#Dummy :{#} Set) where
  ...
  module NaturalityContProven {ℓ :{¶} Level} where
    ...
    open Naturality mkZ[ mkA fibred-over mkB by mf ] mkB zget-image (refl _)
      renaming (naturality to naturality-get-image)

    open Naturality mkZ[ mkA fibred-over mkB by mf ] mkA zunfibre (refl _)
      renaming (naturality to naturality-unfibre)

    naturality :{¶} f ⊚ fold mkA ≡ fold mkB
    naturality =
      (f ⊚ fold mkA ≡ f ⊚ zunfibre ⊚ fold mkZ[ mkA fibred-over mkB by mf ]) ∋
        cong (λ g → f ⊚ g) (sym naturality-unfibre) •
      (f ⊚ zunfibre ⊚ fold mkZ[ mkA fibred-over mkB by mf ] ≡ zget-image ⊚ fold mkZ[ mkA fibred-over mkB by mf ]) ∋
        cong (λ g → g ⊚ fold mkZ[ mkA fibred-over mkB by mf ]) (zfib-lemma f) •
      (zget-image ⊚ fold mkZ[ mkA fibred-over mkB by mf ] ≡ fold mkB) ∋
        naturality-get-image
  ...
  module LoweringLemma where
    module Core {ℓ :{¶} Level} {X :{#} ZSet (ℓ ⊔ ℓZ)} (mkX : IsAlg X) where
      ...
      open NaturalityCont {lsuc (ℓ ⊔ ℓZ)} mkMu (mkLift mkX) lift∘fold (refl _)
  ...
  module InitialityProven {ℓ :{¶} Level} where
    ...
    open NaturalityCont {lsuc (ℓ ⊔ ℓZ)} mkMu mkB f (refl _)
    naturality' :{¶} f ⊚ fold mkMu ≡ fold mkB
    naturality' = naturality
